...
- **GitHub Actions** is the default continuous integration gate. It runs on pushes and pull requests to `main`: install Python dependencies, run pytest, validate Terraform and Compose files, and lint shell scripts. Results show up in GitHub; contributors and forks do not need your Jenkins server. Jobs usually run on **GitHub-hosted runners** in GitHub’s cloud. To let those jobs talk to your deploy targets or sign artifacts, you typically store credentials in **GitHub Actions encrypted secrets**. GitHub holds and injects those values at runtime. That pattern is standard and works well for many teams; the tradeoff is that sensitive material is managed by a third party (GitHub), which some security or compliance policies do not allow for certain keys or environments.
- **Jenkins** is optional. It fits teams that already use Jenkins or want a pipeline that checks out code, tests it, then deploys over SSH to EC2 from `main`. You can run Jenkins **on infrastructure you control** (for example a VM in your own network or next to the app server). SSH keys, API tokens, and deploy secrets can stay in Jenkins credential storage or another system on your side and **never** need to be copied into GitHub’s secret store. That is one reason Jenkins remains attractive when policy says production credentials must not live in a hosted SaaS. The test stage overlaps Actions deliberately so the repo can show both a GitHub-native workflow and a self-contained Jenkins deploy.
